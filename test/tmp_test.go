...
func init() {
	os.Setenv("OS_AUTH_URL", os.Getenv("OS_AUTH_URL"))
	os.Setenv("OS_USERNAME", os.Getenv("OS_USERNAME"))
	os.Setenv("OS_PASSWORD", os.Getenv("OS_PASSWORD"))
	os.Setenv("OS_DOMAIN_NAME", os.Getenv("OS_DOMAIN_NAME"))
}

func TestInterfaces(t *testing.T) {
	log.Println(wooshtools.FuncName())
	...
	for _, link := range links {
		...
		for _, addr := range addrs {
			log.Printf("%v %v", addr.Network(), addr.String())
		}
	}
}
...
func TestPortsCRUD(t *testing.T) {
	...
	tools.PrintResource(t, newSubnet)
	th.AssertEquals(t, newSubnet.Name, subnet.Name)
	// Create port
	port, err := networking.CreatePort(t, client, newSubnet.NetworkID, newSubnet.ID)
	th.AssertNoErr(t, err)
...
func TestPortsWithNoSecurityGroup(t *testing.T) {
	...
	// Create port
	port, err := networking.CreatePortWithNoSecurityGroup(t, client, network.ID, subnet.ID)
	th.AssertNoErr(t, err)
	defer networking.DeletePort(t, client, port.ID)
...
func TestPortsPortSecurityCRUD(t *testing.T) {
	...
	// Create port
	port, err := networking.CreatePortWithoutPortSecurity(t, client, network.ID, subnet.ID)
	th.AssertNoErr(t, err)
	defer networking.DeletePort(t, client, port.ID)
...
func TestPortsWithExtraDHCPOptsCRUD(t *testing.T) {
	...
	port, err := networking.CreatePortWithExtraDHCPOpts(t, client, network.ID, subnet.ID)
	th.AssertNoErr(t, err)
	defer networking.DeletePort(t, client, port.ID)
	...
	newPort := &networking.PortWithExtraDHCPOpts{}
